...
async def fetch_and_parse_feed(client, url):
    ...
    try:
        response = await client.get(url, headers={'User-Agent': 'Fuli-AI/1.0'}, timeout=5.0)
...
def register(mcp):
    ...
    async def get_world_news() -> str:
        ...
        if not all_articles:
            return "The global news grid is unresponsive right now. I'm unable to pull headlines."
    ...
    async def get_world_finance_news() -> str:
        ...
        if not all_articles:
            return "The financial feeds are unresponsive right now. I'm unable to pull market headlines."
    ...
    async def open_world_monitor() -> str:
        ...
        try:
            webbrowser.open(url)
            return "Displaying the World Monitor on your screen now."
    ...
    async def open_finance_world_monitor() -> str:
        ...
        try:
            webbrowser.open(url)
            return "Displaying the Finance World Monitor on your screen now."
        except Exception as e:
